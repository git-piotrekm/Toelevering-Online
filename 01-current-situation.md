...
- **Karta 1 — Delivery** (card-soft)
  Ikona (indigo): 🔀 — strzałki w kółko / chaos przepływu (np. `heroicons: arrow-path` lub `arrows-pointing-in`)
  4 zespoły dev + PO przeciążeni bez BA i business ownerów. Refinement tylko 1 sprint do przodu. Scrum bez Scrum Masterów.

- **Karta 2 — QA** (card-ind)
  Ikona (biała / indigo-glow na karcie indigo): 🐛 — robak / błąd (np. `heroicons: bug-ant`)
...
- **Karta 3 — AI** (card-dark)
  Ikona (indigo-glow na ciemnej karcie): ⚡ — błyskawica / brak kontroli (np. `heroicons: bolt`)
